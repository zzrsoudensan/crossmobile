...
/*
 * This class is heavily dependent on the 
 */
 ...
 */
public class IOSScrollView extends IOSView {

    // by teras
    private boolean pagingEnabled = false;
    private CGSize contentSize = null;
    //
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
                if (pagingEnabled) {
                    ...
                    int ypage = (int) (yscroll / pageframe.size.height + 0.5f);

                    // Check that we are inside right bounds
                    CGSize csize = getContentSize();
                    float toX = (xpage + 1) * pageframe.size.width; // end of area
                    float toY = (ypage + 1) * pageframe.size.height;  // end of area
                    if (toX > csize.width)
                        toX = csize.width;
                    if (toY > csize.height)
                        toY = csize.height;

                    // Check that we are inside left bounds (only if right bounds are taken into account)
                    toX -= pageframe.size.width;  // begin of area
                    toY -= pageframe.size.height;  // begin of area
                    if (toX < 0)
                        toX = 0;
                    if (toY < 0)
                        toY = 0;
    ...
    public CGSize getContentSize() {
        if (contentSize == null)
            return ((IOSView.LayoutParams) getLayoutParams()).getFrame().size;
        return contentSize;
    }
}
